...
const Counter = ({ value, suffix }: { value: number; suffix: string }) => {
  ...
  return (
    <div ref={ref} className="font-display text-[clamp(2.5rem,6vw,5rem)] font-bold leading-none text-gradient-primary">
      {v}<span className="text-foreground">{suffix}</span>
    </div>
  );
};

const Stats = () => {
  const { data } = useSiteContent("stats");
  return (
        ...
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
          {data.items.map((s, i) => (
            <div key={s.label + i} className="text-center">
              ...
              <div className="mt-3 font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
